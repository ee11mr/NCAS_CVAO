...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov 18 11:42:18 2019
Script to look at trends in ozone precursors
@author: mjr583
"""
...
species='$CH_4$'
nox = df['NO (pptV)'][start_year:] + df['NO2 (pptV)'][start_year:]
no = df['NO (pptV)'][start_year:] 
no2 = df['NO2 (pptV)'][start_year:] 
o3 = df['O3'][start_year:] 
co = df['CO (ppbV)'][start_year:] 
ch4 = df['CH4 all (ppbV)'][start_year:] 
vocs = df['Isoprene (pptV)'][start_year:] 
co2 = df['CO2 (ppmV)'][start_year:]
temp = df['TEMP_10M (degC)'][start_year:]
o3 = df['O3'][start_year:]
...
if species == 'CO':
    ...
    o3['2009-07-01' : '2009-09-30'] = np.nan
    data = o3
    unit = 'ppbv'
...
for i in range(len(ds)):
    ...
    #plt.ylim(13,44)
plt.tight_layout()
plt.savefig(savepath+'/trends_co.png')
plt.close()
...
for dn,d in enumerate(ds):
    ...
    r2 = np.round((r2_score(y,y_poly_pred) * 100),1)
    ...
    ax.set_ylabel('Monthly '+labels[dn]+' '+species+' ('+unit+')')
    years=np.arange(2007,2020)
    plt.xticks(np.arange(0, len(x), 12), years)
    '''
    ...
    '''
    plt.savefig(savepath+'ozone_trends/'+species+'_'+labels[dn][:4]+'_poly_trend.pdf', dpi=300)
    plt.close()
